...
class DecomposeRule(ABC):
    ...
    def _validate_preserved(self, doc: [Token | ReferenceToken], span: (int, int)) -> bool:
        ...
        for preserved in self._preserved_tokens:
            ...
            post_offset = 1 if preserved.endswith(' ') and end + 1 < len(
                doc) else 0  # make sure its not: [... preserved]
...
class RemoveByConditionDecomposeRule(DecomposeRule):
    ...
    def _decompose(self, node_id: int, graph: nx.DiGraph, doc: [Token]) -> [int]:
        to_keep = [token for i, token in enumerate(doc) if
                   (isinstance(token, ReferenceToken) or not self.condition(token)
    ...
    def _decompose(self, node_id: int, graph: nx.DiGraph, doc: [Token]) -> [int]:
        ...
        for i, token in enumerate(doc):
            if token.pos_ == "ADJ":
                ...
                elif i > 1 and doc[i - 1].lemma_ == "be" and isinstance(doc[i - 2],
                                                                        ReferenceToken):  # doc[i-2].tag_.startswith("NN"):
                    # NN is ADJ
                    extract_spans.append((i - 2, i))
                elif i > 2 and doc[i - 1].lemma_ == "be" and doc[i - 2].pos_ == "DET" and isinstance(doc[i - 3],
                                                                                                     ReferenceToken):  # doc[i-3].tag_.startswith("NN"):
                    # NN that is ADJ
    ...
    def _decompose(self, node_id: int, graph: nx.DiGraph, doc: [Token]) -> [int]:
        ...
        for i, token in enumerate(doc):
            if token.tag_.startswith("NN"):
                if i > 1 and doc[i - 1].lemma_ == "be" and isinstance(doc[i - 2],
                                                                      ReferenceToken):  # doc[i-2].tag_.startswith("NN"):
                    # NN is NN
                    extract_spans.append((i - 2, i))
                elif i > 2 and doc[i - 1].lemma_ == "be" and doc[i - 2].pos_ == "DET" and isinstance(doc[i - 3],
