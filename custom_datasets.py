...
class BREAKLogical(data.Dataset):
    ...
    def __init__(self, data_dir, train=True, valid=False):
        ...
        # required for compositional generalization split by operator location
        self.operator_list = self.dataset_logical[dataset_type]['operators']
        # required for compositional generalization split by modality of
        # pattern is <dataset_name>_<dataset_type(train/valid/test)>_<index>
        self.question_ids = self.dataset_logical[dataset_type]['question_id']
        # str of list for each step
        self.programs = self.dataset_logical[dataset_type]['program']

    def __getitem__(self, idx):
        example = (self.questions[idx], self.golds[idx], self.lexicon_dict[idx], self.programs[idx])
        return example
